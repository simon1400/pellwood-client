...
const Delivery = ({
  ...
  error,
  onBlur,
  setError}: { data: any, setData: any, error: any, onBlur: any, setError: any }) => {

  const router = useRouter()
  const {lang} = localize(router.locale) as { lang: 'cz' | 'en' | 'de' }
        ...
        <div className="select_item">
          <div uk-form-custom="target: > * > span:first-child">
            <select value={state.country} onChange={(e) => handleChange('country', e.target.value)} >
              {(countryData[lang as keyof typeof countryData] || countryData['en']).map((item, index) => <option key={index} value={item.name}>{item.value}</option>)}
            </select>
